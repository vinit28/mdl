...
def draw_polygons( points, screen, color ):
    ...
    while i < len(points):
        ...
        p1 = points[i+1]
        p2 = points[i+2]
        N = [(p1[1]*p2[2] - p1[2]*p2[1]),(p1[2]*p2[0] - p1[0]*p2[2]),(p1[0]*p2[1] - p1[1]*p2[0])]
        V = [0,0,-1]
        ifdraw = N[0]*V[0] + N[1]*V[1] + N[2]*V[2]
        if ifdraw < 0:
...
def add_box( points, x, y, z, width, height, depth ):
    ...
    add_polygon(points, x, y, z, x, y1, z, x1, y, z) #front
    add_polygon(points, x1, y, z, x, y1, z, x1, y1, z) 
    add_polygon(points, x1, y, z, x1, y1, z, x1, y, z1) #right
    add_polygon(points, x1, y, z1, x1, y1, z, x1, y1, z1)
    add_polygon(points, x, y, z1, x, y, z, x1, y, z1) #top
    add_polygon(points, x1, y, z1, x, y, z, x1, y, z)
    add_polygon(points, x, y1, z, x, y1, z1, x1, y1, z) #bottom
    add_polygon(points, x1, y1, z, x, y1, z1, x1, y1, z1)
    add_polygon(points, x1, y, z1, x1, y1, z1, x, y, z1) #back
    add_polygon(points, x, y, z1, x1, y1, z1, x, y1, z1)
    add_polygon(points, x, y, z1, x, y1, z1, x, y, z) #left
...
def add_sphere( points, cx, cy, cz, r, step ):
    ...
    while lat <= lat_stop:
        ...
        while longt < longt_stop:
            ...
            indexu = lat * num_steps + longt #upper row
            indexl = (lat+1) * num_steps + longt #lower row
            add_polygon(points,temp[indexu][0],temp[indexu][1],temp[indexu][2],
                        temp[indexl][0],temp[indexl][1],temp[indexl][2],
                        temp[indexl+1][0],temp[indexl+1][1],temp[indexl+1][2])
            add_polygon(points,temp[indexu][0],temp[indexu][1],temp[indexu][2],
                        temp[indexl+1][0],temp[indexl+1][1],temp[indexl+1][2],
                        temp[indexu+1][0],temp[indexu+1][1],temp[indexu+1][2])
            longt+= 1
        lat+= 1
